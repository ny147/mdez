...
Date: 2026-09-06. Task: redesign the live Mdez workspace and prepare an implementation spec from the accepted single-page HTML.
...
- Checkpoint commit is the commit containing this file, titled `docs: checkpoint redesign reference and implementation spec`. Resolve with `git log -1 --format="%h %s" -- docs/REDESIGN-RESUME.md`.
...
New personal bookmarks require a separate additive Dexie v5 table, scoped to local/group workspace IDs; they do not modify shared document records. Keep current 767px/1023px responsive boundaries. No deployment is authorized by this planning task.
