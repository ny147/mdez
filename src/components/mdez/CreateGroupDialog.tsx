...
export function CreateGroupDialog({ open, folders, documents, onClose, onCreated, generateKey: makeKey = generateGroupKey }: Props) {
  ...
  return <ModalDialog title="Create Key Group" titleId="create-key-group-title" closeLabel="Close group creation" onClose={onClose}>
    <div className="mt-5 space-y-4">
      ...
      {createdKey ? <div className="space-y-3"><label className="block text-sm font-bold">Group key<input aria-label="Group key" readOnly value={createdKey} className="mt-2 w-full rounded border border-border bg-surface px-3 py-2 font-mono text-sm" /></label><div className="flex gap-2"><button type="button" onClick={() => navigator.clipboard.writeText(createdKey)} className="rounded bg-accent px-4 py-2 font-bold text-accent-on">Copy key</button><button type="button" onClick={download} className="rounded border border-border bg-surface px-4 py-2 font-bold">Download .txt</button></div></div> : <button type="button" disabled={busy || !name.trim()} onClick={() => void create()} className="rounded bg-accent px-4 py-2 font-bold text-accent-on disabled:opacity-50">{busy ? "Creating group..." : "Create and copy Local Library"}</button>}
    </div>
  </ModalDialog>;
}
