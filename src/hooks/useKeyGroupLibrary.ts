...
export function useKeyGroupLibrary(groupId: string | null): WorkspaceController {
  ...
  const selectDocument = useCallback((id: string) => { const document = documents.find((item) => item.id === id); if (!document) return; setSelectedDocumentId(id); setSelectedFolderId(document.folderId); const next = document.folderId ?? UNSORTED_COLLECTION_ID; setExpandedCollectionId(next); if (groupId) saveExpandedCollection(groupId, next); }, [documents, groupId]);
  ...
  const createPage = useCallback(async (targetFolderId = selectedFolderId) => { if (!groupId) return; const created = await createGroupDocument(groupId, keyRef.current, { title: "untitled.md", body: "# Untitled\n", folderId: targetFolderId }); documentVersions.current.set(created.id, created.version); setDocuments((current) => [...current, documentView(created)]); setSelectedFolderId(targetFolderId); setSelectedDocumentId(created.id); const next = targetFolderId ?? UNSORTED_COLLECTION_ID; setExpandedCollectionId(next); saveExpandedCollection(groupId, next); }, [groupId, selectedFolderId]);
  const importPages = useCallback(async (items: { title: string; body: string }[], folderId: string | null) => { if (!groupId) return; let newest: GroupDocument | null = null; for (const item of items) { newest = await createGroupDocument(groupId, keyRef.current, { ...item, folderId }); documentVersions.current.set(newest.id, newest.version); setDocuments((current) => [...current, documentView(newest!)]); } const next = folderId ?? UNSORTED_COLLECTION_ID; setSelectedFolderId(folderId); setSelectedDocumentId(newest?.id ?? null); setExpandedCollectionId(next); saveExpandedCollection(groupId, next); }, [groupId]);
