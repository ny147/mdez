...
export function useKeyGroupLibrary(groupId: string | null): WorkspaceController {
  ...
  const createPage = useCallback(async () => { if (!groupId) return null; const created = await createGroupDocument(groupId, keyRef.current, { title: "untitled.md", body: "# Untitled\n", folderId: selectedFolderId }); const view = documentView(created); documentVersions.current.set(created.id, created.version); setDocuments((current) => [...current, view]); setSelectedDocumentId(created.id); return view; }, [groupId, selectedFolderId]);
