...
export function useKeyGroupLibrary(groupId: string | null): WorkspaceController {
  ...
  const [selectedFolderId, setSelectedFolderId] = useState<string | null>(null); const [selectedDocumentId, setSelectedDocumentId] = useState<string | null>(null); const [expandedCollectionId, setExpandedCollectionId] = useState<string | null>(null);
  ...
  const keyRef = useRef(""); const folderVersions = useRef(new Map<string, number>()); const documentVersions = useRef(new Map<string, number>()); const expansionWorkspaceRef = useRef<string | null>(null);
  ...
  const apply = useCallback((snapshot: GroupSnapshot, preferred?: string | null) => {
    ...
    const validBookIds = new Set(snapshot.folders.map((folder) => folder.id));
    setExpandedCollectionId((current) => {
      if (expansionWorkspaceRef.current !== snapshot.group.id) {
        expansionWorkspaceRef.current = snapshot.group.id;
        return loadExpandedCollection(snapshot.group.id, validBookIds);
      }
      return current === UNSORTED_COLLECTION_ID || (current !== null && validBookIds.has(current)) ? current : null;
    });
  ...
  useEffect(() => {
    let alive = true; setIsReady(!groupId); setError(null); setPageReveal(null);
  ...
  const selectDocument = useCallback((id: string) => { const document = documents.find((item) => item.id === id); if (!document) return; setSelectedDocumentId(id); setPageReveal((current) => ({ documentId: id, sequence: (current?.sequence ?? 0) + 1 })); setSelectedFolderId(document.folderId); const next = document.folderId ?? UNSORTED_COLLECTION_ID; setExpandedCollectionId(next); if (groupId) saveExpandedCollection(groupId, next); }, [documents, groupId]);
  const toggleFolder = useCallback((id: string) => setExpandedCollectionId((current) => { const next = current === id ? null : id; if (groupId) saveExpandedCollection(groupId, next); return next; }), [groupId]);

  const createBook = useCallback(async (parentId: string | null) => { void parentId; if (!groupId) return; const name = window.prompt("New book name", "New Book"); if (name === null) return; try { const created = await createGroupFolder(groupId, keyRef.current, { name, parentId: null }); folderVersions.current.set(created.id, created.version); setFolders((current) => [...current, folderView(created)]); setSelectedFolderId(created.id); setExpandedCollectionId(created.id); saveExpandedCollection(groupId, created.id); } catch { setError("Could not create book."); } }, [groupId]);
  ...
  const deleteBook = useCallback(async (id: string) => { if (!groupId) return; const book = folders.find((item) => item.id === id); if (!book) return; const count = documents.filter((item) => item.folderId === id).length; if (!window.confirm(count ? `Delete “${book.name}”? Its ${count} ${count === 1 ? "page" : "pages"} will move to Unsorted. No pages will be deleted.` : `Delete “${book.name}”?`)) return; try { await deleteGroupFolder(groupId, keyRef.current, id, folderVersions.current.get(id) ?? 1); await fullRefresh(); if (selectedFolderId === id) setSelectedFolderId(null); setExpandedCollectionId((current) => { const next = current === id || selectedFolderId === id ? UNSORTED_COLLECTION_ID : current; if (next !== current) saveExpandedCollection(groupId, next); return next; }); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not delete book."); } }, [documents, folders, fullRefresh, groupId, selectedFolderId]);
  const createPage = useCallback(async (targetFolderId = selectedFolderId) => { if (!groupId) return; const created = await createGroupDocument(groupId, keyRef.current, { title: "untitled.md", body: "# Untitled\n", folderId: targetFolderId }); documentVersions.current.set(created.id, created.version); setDocuments((current) => [...current, documentView(created)]); setSelectedFolderId(targetFolderId); setSelectedDocumentId(created.id); setPageReveal((current) => ({ documentId: created.id, sequence: (current?.sequence ?? 0) + 1 })); const next = targetFolderId ?? UNSORTED_COLLECTION_ID; setExpandedCollectionId(next); saveExpandedCollection(groupId, next); }, [groupId, selectedFolderId]);
  const importPages = useCallback(async (items: { title: string; body: string }[], folderId: string | null) => { if (!groupId) return; let newest: GroupDocument | null = null; for (const item of items) { newest = await createGroupDocument(groupId, keyRef.current, { ...item, folderId }); documentVersions.current.set(newest.id, newest.version); setDocuments((current) => [...current, documentView(newest!)]); } const next = folderId ?? UNSORTED_COLLECTION_ID; setSelectedFolderId(folderId); setSelectedDocumentId(newest?.id ?? null); if (newest) setPageReveal((current) => ({ documentId: newest!.id, sequence: (current?.sequence ?? 0) + 1 })); setExpandedCollectionId(next); saveExpandedCollection(groupId, next); }, [groupId]);
  ...
  const movePage = useCallback(async (id: string, folderId: string | null) => { if (!groupId) return; try { const updated = await updateGroupDocument(groupId, keyRef.current, id, { folderId, expectedVersion: documentVersions.current.get(id) ?? 1 }); documentVersions.current.set(id, updated.version); setDocuments((current) => current.map((item) => item.id === id ? documentView(updated) : item)); if (selectedDocumentId === id) { const next = folderId ?? UNSORTED_COLLECTION_ID; setSelectedFolderId(folderId); setExpandedCollectionId(next); setPageReveal((current) => ({ documentId: id, sequence: (current?.sequence ?? 0) + 1 })); saveExpandedCollection(groupId, next); } setError(null); } catch (cause) { setError(cause instanceof Error ? cause.message : "Could not move page."); throw cause; } }, [groupId, selectedDocumentId]);
